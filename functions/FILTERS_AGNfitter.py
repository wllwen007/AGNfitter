...
class FILTER_SET:
    ...
    def __init__(self, filterset_name, filtersdict, filters_objects_all):
        ...
        if filtersdict['filterset'] == 'filterset_default':
         ...
         'Y_VISTA', 'z_SUBARU', 'i_SUBARU', 'r_SUBARU', 'B_SUBARU', 'u_CHFT',\
          'GALEX_2500']
        ...
        elif filtersdict['filterset'] == 'filterset_file':
            if filtersdict['file']:
                if not os.path.exists(filtersdict['file']):
                    print('Error: specified filter file list', filtersdict['file'],' does not exist')
                    sys.exit(1)
            else:
                print('Error:  filterset is set to filterset_file but no file is specified')
                sys.exit(1)
            self.set_filters_from_file(filtersdict['file'])
            ...
            ADDfilters=dict()
            ADDfilters['names'] = self.filternames    ## (string/list of strings)User especified filter names. 
                                        ## If name has been previously used, an error message will appear. 
            ADDfilters['filenames'] =self.filterfilenames ## (string/list of strings) File names of the new filters. 
            ...
            filtersdict['add_filters_dict']= ADDfilters
            filters_objects_chosen = None
    ...
    def set_filters_from_file(self, filename):
    
        print('reading filters from: '+filename)
        files =[]
        lambdas = []
        factors = []
        ...
        filternames = np.loadtxt(filename, delimiter = '|', usecols=[0], skiprows = 1, dtype=str)
        filterfiles = np.loadtxt(filename, delimiter = '|', usecols=[1], skiprows = 1, dtype=str)
        centralwls = np.loadtxt(filename, delimiter = '|', usecols=[3], skiprows = 1)
        ...
        self.filterfilenames = filterfiles
        self.central_nu_array = np.array(sorted(central_nu_list))
        self.lambdas_dict = lambdas_dict
        self.factors_dict = factors_dict
...
def create_filtersets(filters_dict, path):
    ...
    """   
    if filters_dict['filterset'] == 'filterset_file':
        filterset = FILTER_SET(filters_dict['filterset'], filters_dict, None)
        return filterset
