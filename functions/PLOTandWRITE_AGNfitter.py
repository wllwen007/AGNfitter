...
class OUTPUT:
    ...
    def write_parameters_outputvalues(self, P):        
        ...
        Mstar0, SFR_opt0 = model.stellar_info_array(np.array([self.chain.best_fit_pars]), self.data, self.models, 1)
    ...
    def plot_manyrealizations_SED(self, plot_residuals=True):    
        ...
        #plotting settings
        if plot_residuals:
            fig, ax1, ax2, axr = SED_plotting_settings_ar(all_nus_rest, data_nuLnu_rest, self.allnus)
        else:
...
class CHAIN:
    ...
    def plot_trace(self, P, nwplot=50):
        ...
        for i in range(npar):
            ...
            ax.set_title(r'Parameter : '+P['names'][i], fontsize=12)  
            ax.set_xlabel(r'Steps', fontsize=12)
            ax.set_ylabel(r'Walkers',fontsize=12)
...
#>>>>>>> AGNfitter_v2.0
    ...
    def integrated_luminosities(self,out ,all_nus_rest, nuLnus4plotting):
        ...
        try:
            ...
            print ('Error converting units, assuiming it is ok')
            print(out['intlum_freqranges'][0][0].unit)
        int_lums = []
...
def SED_plotting_settings_ar(x, ydata, modeldata):
